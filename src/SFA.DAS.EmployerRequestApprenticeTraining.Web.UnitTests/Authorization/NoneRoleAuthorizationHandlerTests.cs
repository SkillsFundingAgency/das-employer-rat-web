...
{
    public class NoneRoleAuthorizationHandlerTests
    {
        [Test, MoqAutoData]
        ...
        {
            // Arrange
            var requirement = new NoneRoleRequirement();
            var user = new ClaimsPrincipal(new ClaimsIdentity(new Claim[] { new Claim(ClaimTypes.Name, "Test User") }));
            var context = new AuthorizationHandlerContext(new[] { requirement }, user, null);

            employerRoleAuthorizationService.Setup(x => x.IsEmployerAuthorized(It.IsAny<ClaimsPrincipal>(), UserRole.None))    
                .ReturnsAsync(true);

            // Act
            var handler = new NoneRoleAuthorizationHandler(employerRoleAuthorizationService.Object);
