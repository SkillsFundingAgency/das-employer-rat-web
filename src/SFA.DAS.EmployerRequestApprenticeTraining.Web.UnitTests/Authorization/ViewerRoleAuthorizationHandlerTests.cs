...
{
    public class ViewerRoleAuthorizationHandlerTests
    {
        [Test, MoqAutoData]
        ...
        {
            // Arrange
            var requirement = new ViewerRoleRequirement();
            var user = new ClaimsPrincipal(new ClaimsIdentity(new Claim[] { new Claim(ClaimTypes.Name, "Test User") }));
            var context = new AuthorizationHandlerContext(new[] { requirement }, user, null);

            employerRoleAuthorizationService.Setup(x => x.IsEmployerAuthorized(It.IsAny<ClaimsPrincipal>(), UserRole.Viewer))    
                .ReturnsAsync(true);

            // Act
            var handler = new ViewerRoleAuthorizationHandler(employerRoleAuthorizationService.Object);
